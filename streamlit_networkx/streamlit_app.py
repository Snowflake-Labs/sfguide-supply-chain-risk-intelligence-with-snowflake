"""
Supply Chain Risk Analysis - Storytelling Dashboard (NetworkX)
...
st.markdown("""
<style>
    /* Dark theme foundation - force dark background */
...
def load_key_metrics(_session):
    ...
    queries = {
        'total_nodes': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.RISK_SCORES_NX",
        'critical_count': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.RISK_SCORES_NX WHERE RISK_CATEGORY = 'CRITICAL'",
        'bottleneck_count': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.BOTTLENECKS_NX",
...
def load_illusion_data(_session):
    ...
    # Get top bottleneck (the common Tier-2 supplier)
    try:
...
def render_illusion_graph(nodes, edges, height=500):
    """Render the 'illusion of diversity' graph showing unknown convergence using Plotly."""
...
def main():
    ...
    with col3:
        st.metric(
            ...
            help="Unknown single points of failure"
        )
    
    with col4:
        st.metric(
            label="Inferred Links Discovered",
            value=f"{metrics['predicted_links']:,}",
            help="Tier-2+ relationships predicted by network analysis"
        )
    ...
    if metrics['total_nodes'] == 0:
        ...
        <div style="background: linear-gradient(135deg, #1e3a5f 0%, #0f172a 100%); 
            ...
            <div style="font-size: 1.2rem; color: #f8fafc; margin-bottom: 0.5rem;">
                <strong>Run the NetworkX Analysis</strong>
            </div>
            <div style="color: #94a3b8; margin-bottom: 1rem;">
                Execute the notebook to generate risk scores, discover unknown dependencies, and identify bottlenecks.
            </div>
            <a href="../notebooks/SUPPLY_CHAIN_RISK.SUPPLY_CHAIN_RISK.SUPPLY_CHAIN_RISK_NX_NOTEBOOK" 
               target="_blank"
               style="display: inline-block; background: #3b82f6; color: white; 
                      padding: 0.75rem 2rem; border-radius: 8px; text-decoration: none;
                      font-weight: 600; transition: background 0.2s;">
                Open NetworkX Notebook →
            </a>
        </div>
        """, unsafe_allow_html=True)
    ...
    with st.expander("External Trade Intelligence", expanded=False):
        st.markdown("""
        <p style="color: #94a3b8; margin-bottom: 1rem;">
            Shipping and trade data reveals external entities supplying your vendors — potential common Tier-2 suppliers.
        </p>
        """, unsafe_allow_html=True)
    ...
    <div style="background: linear-gradient(135deg, rgba(59, 130, 246, 0.1) 0%, rgba(139, 92, 246, 0.1) 100%);
        ...
        <p style="color: #e2e8f0;">
            ...
            your suppliers. Multiple Tier-1 vendors may depend on the same common Tier-2 source — 
            creating concentration risk that standard analytics miss.
        </p>
        <p style="color: #94a3b8; margin-top: 0.5rem; font-size: 0.9rem;">
            The network analysis below reveals these unknown dependencies.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div class="problem-box">
            ...
            <p>
                ...
                creating unknown concentration risks.
            </p>
    ...
    if bottleneck and len(nodes) > 1:
        ...
        <p style="text-align: center; color: #64748b; font-size: 0.9rem; margin-top: 1rem;">
            ...
            <strong>Red node:</strong> common Tier-2 supplier (discovered by network analysis) &nbsp;|&nbsp;
            <strong>Dashed lines:</strong> Predicted dependencies
        </p>
        """, unsafe_allow_html=True)
    else:
        st.info("Run the NetworkX notebook to discover unknown dependencies in your supply chain data.")
    ...
    with col2:
        ...
        <div class="narrative-card">
            ...
            <p>
                ...
                Tier-2+ sources, creating unknown concentration risk.
            </p>
        </div>
        """, unsafe_allow_html=True)
