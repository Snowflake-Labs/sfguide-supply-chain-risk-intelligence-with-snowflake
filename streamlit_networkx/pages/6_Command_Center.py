...
def load_alert_summary(_session):
    ...
    queries = {
        'critical_count': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.RISK_SCORES_NX WHERE RISK_CATEGORY = 'CRITICAL' AND NODE_TYPE = 'SUPPLIER'",
        'high_count': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.RISK_SCORES_NX WHERE RISK_CATEGORY = 'HIGH' AND NODE_TYPE = 'SUPPLIER'",
        'medium_count': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.RISK_SCORES_NX WHERE RISK_CATEGORY = 'MEDIUM' AND NODE_TYPE = 'SUPPLIER'",
        'bottleneck_count': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.BOTTLENECKS_NX WHERE IMPACT_SCORE >= 0.5"
    }
...
def main():
    ...
    if not watchlist.empty:
        ...
        st.info("No suppliers currently on watchlist. Run the NetworkX notebook to generate risk scores.")
