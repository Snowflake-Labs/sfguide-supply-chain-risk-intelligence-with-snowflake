...
def load_high_risk_suppliers(_session, limit=15):
    ...
    try:
        result = _session.sql(f"""
            ...
            FROM {DB_SCHEMA}.RISK_SCORES_NX rs
...
def load_recommended_actions(_session):
    ...
    if bottlenecks is not None and not bottlenecks.empty:
        for _, row in bottlenecks.iterrows():
            actions.append({
                ...
                "description": f"This common Tier-2 supplier affects {row['DEPENDENT_COUNT']} of your vendors. "
    ...
    if len(actions) == 2:  # Only the two routine actions
        actions.insert(0, {
            "priority": "important",
            "title": "Run NetworkX analysis",
            "description": "Execute the NetworkX notebook to generate risk scores and predictions.",
            "impact_score": 0.5,
            "action_type": "routine"
        })
...
Action: {action['title']}
Context: 
- This is a common Tier-2 supplier that {action.get('dependent_count', 0)} of our Tier-1 vendors depend on
...
def execute_query(session, question):
    ...
    query_map = {
        ...
        "average risk": """
            ...
            LEFT JOIN RISK_SCORES_NX rs ON v.VENDOR_ID = rs.NODE_ID
        ...
        "material": """
            ...
            FROM MATERIALS m
            LEFT JOIN RISK_SCORES_NX rs ON m.MATERIAL_ID = rs.NODE_ID
...
def main():
    ...
    with col2:
        try:
            bottlenecks = session.sql("SELECT * FROM BOTTLENECKS_NX ORDER BY IMPACT_SCORE DESC").to_pandas()
    ...
    with col3:
        try:
            links = session.sql("SELECT * FROM PREDICTED_LINKS_NX WHERE PROBABILITY >= 0.5 ORDER BY PROBABILITY DESC").to_pandas()
            if not links.empty:
