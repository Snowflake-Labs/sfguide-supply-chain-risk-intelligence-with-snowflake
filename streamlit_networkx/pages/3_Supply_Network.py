...
def load_graph_data(_session, include_predicted=True):
    ...
    try:
        materials = _session.sql(f"""
            ...
            LEFT JOIN {DB_SCHEMA}.RISK_SCORES_NX rs ON m.MATERIAL_ID = rs.NODE_ID
        """).to_pandas()
...
def load_graph_stats(_session):
    ...
    queries = {
        ...
        'predicted_links': "SELECT COUNT(*) as CNT FROM PREDICTED_LINKS_NX",
        'external_suppliers': "SELECT COUNT(DISTINCT SOURCE_NODE_ID) as CNT FROM PREDICTED_LINKS_NX"
    }
...
def main():
    ...
    if nodes:
        ...
        st.info("No graph data available. Run the NetworkX notebook to build the knowledge graph.")
