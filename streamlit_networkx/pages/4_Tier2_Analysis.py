...
def load_discovery_summary(_session):
    ...
    queries = {
        'total_bottlenecks': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.BOTTLENECKS_NX",
        'critical_bottlenecks': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.BOTTLENECKS_NX WHERE IMPACT_SCORE >= 0.7",
        'predicted_links': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.PREDICTED_LINKS_NX",
        'high_confidence_links': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.PREDICTED_LINKS_NX WHERE PROBABILITY >= 0.7",
        'total_dependencies': f"SELECT COALESCE(SUM(DEPENDENT_COUNT), 0) as CNT FROM {DB_SCHEMA}.BOTTLENECKS_NX"
    }
    ...
    try:
        result = _session.sql(f"""
            SELECT 
                ...
                pl.PROBABILITY,
                rs.RISK_SCORE
            FROM {DB_SCHEMA}.PREDICTED_LINKS_NX pl
            LEFT JOIN {DB_SCHEMA}.VENDORS v ON pl.TARGET_NODE_ID = v.VENDOR_ID
            LEFT JOIN {DB_SCHEMA}.RISK_SCORES_NX rs ON v.VENDOR_ID = rs.NODE_ID
    ...
    try:
        result = _session.sql(f"""
            SELECT 
                ...
                EVIDENCE_STRENGTH,
                PREDICTED_AT
            FROM {DB_SCHEMA}.PREDICTED_LINKS_NX
...
Context:
- common Tier-2 Supplier: {context_data.get('node_id', 'Unknown')}
...
Analysis Results:
- Total predicted unknown relationships: {context_data.get('total_links', 0)}
...
Explain what these inferred links mean for supply chain visibility and risk management.
...
def main():
    ...
    with col3:
        ...
        <div class="stat-card">
            ...
            <div class="stat-label">Inferred Links</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if bottlenecks is not None and not bottlenecks.empty:
        ...
        for idx, row in bottlenecks.iterrows():
            ...
            if dep_names:
                chips_html = ''.join([f'<span class="vendor-chip">{name}</span>' for name in dep_names])
                more_html = f'<span style="color: #64748b; font-size: 0.8rem;"> +{len(deps) - 5} more</span>' if deps is not None and len(deps) > 5 else ''
                vendor_section = f'<div class="evidence-section"><div class="evidence-title">Affected Tier-1 Vendors</div>{chips_html}{more_html}</div>'
            else:
                vendor_section = '<div class="evidence-section"><span style="color: #64748b; font-size: 0.85rem;">No affected Tier-1 vendors identified</span></div>'
            
            card_html = f"""<div class="bottleneck-card {impact_class}"><div class="bottleneck-header"><span class="bottleneck-name">{row['NODE_ID']}</span><span class="impact-badge impact-{impact_class}">{impact_label} IMPACT</span></div><div class="bottleneck-stats"><span>Impact: <strong>{impact:.0%}</strong></span><span>Dependents: <strong>{row['DEPENDENT_COUNT']}</strong></span><span>Status: <strong>{row.get('MITIGATION_STATUS', 'UNMITIGATED')}</strong></span></div><div class="bottleneck-description">{row.get('DESCRIPTION', 'Unknown supply chain convergence point requiring attention.')}</div>{vendor_section}</div>"""
        ...
        st.info("No bottlenecks identified. Run the NetworkX notebook to analyze your supply chain.")
