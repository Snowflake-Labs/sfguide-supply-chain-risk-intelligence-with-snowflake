...
def load_data_statistics(_session):
    ...
    table_descriptions = {
        ...
        'PREDICTED_LINKS_NX': 'Unknown relationships discovered by network analysis',
        'RISK_SCORES_NX': 'Nodes with propagated risk scores'
    }
...
def render_data_flow_sankey(stats, height=350):
    ...
    labels = [
        ...
        "Knowledge Graph",
        "NetworkX Model",
        f"Predictions ({stats.get('PREDICTED_LINKS_NX', {}).get('count', 0)})",
        f"Risk Scores ({stats.get('RISK_SCORES_NX', {}).get('count', 0)})"
    ]
    
    # Node indices: 0=ERP, 1=Vendors, 2=Materials, 3=POs, 4=BOM, 5=Trade Src, 6=Trade, 7=Graph, 8=NetworkX, 9=Predictions, 10=Risks
    ...
    values = [
        ...
        max(stats.get('TRADE_DATA', {}).get('count', 1), 1) // 5 or 1,
        50,
        max(stats.get('PREDICTED_LINKS_NX', {}).get('count', 1), 1) // 3 or 1,
        max(stats.get('RISK_SCORES_NX', {}).get('count', 1), 1) // 3 or 1
    ]
    ...
    link_colors = [
        ...
        'rgba(139, 92, 246, 0.4)',  # Graph to NetworkX
        'rgba(16, 185, 129, 0.4)', 'rgba(16, 185, 129, 0.4)'  # NetworkX outputs
    ]
...
def main():
    ...
    <p style="text-align: center; color: #64748b; font-size: 0.85rem; margin-top: 0.5rem;">
        ...
        <span style="color: #10b981;">■</span> Model Outputs
    </p>
    """, unsafe_allow_html=True)
    ...
    with col5:
        ...
        <div style="text-align: center; padding: 1rem;">
            <div style="background: #7f1d1d; border: 2px solid #dc2626; border-radius: 8px; padding: 1rem;">
                ...
                <div style="color: #fca5a5; font-size: 0.9rem;">Unknown Suppliers</div>
    ...
    <p style="color: #94a3b8; margin-bottom: 1rem;">
        Inferred relationships and calculated risk scores from the network analysis.
    </p>
    """, unsafe_allow_html=True)
    ...
    with col1:
        pl = stats.get('PREDICTED_LINKS_NX', {})
        st.markdown(f"""
        <div class="data-card" style="border-color: #10b981;">
            <h3>Predicted Links <span style="background: #166534; color: #fff; padding: 2px 8px; border-radius: 4px; font-size: 0.7rem;">MODEL OUTPUT</span></h3>
            <div class="data-count" style="color: #10b981;">{pl.get('count', 0):,}</div>
            <p>{pl.get('description', '')} — common Tier-2+ relationships discovered by analyzing trade patterns</p>
        </div>
        """, unsafe_allow_html=True)
    
    with col2:
        rs = stats.get('RISK_SCORES_NX', {})
        st.markdown(f"""
        <div class="data-card" style="border-color: #10b981;">
            <h3>Risk Scores <span style="background: #166534; color: #fff; padding: 2px 8px; border-radius: 4px; font-size: 0.7rem;">MODEL OUTPUT</span></h3>
