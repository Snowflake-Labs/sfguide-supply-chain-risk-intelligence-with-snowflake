/******************************************************************************
 * SUPPLY CHAIN RISK ANALYSIS - NETWORKX SETUP SCRIPT (TRIAL ACCOUNT EDITION)
 * 
 * Purpose: Deploy a NetworkX-powered supply chain risk analysis platform on
 *          Snowflake using standard warehouse compute, Cortex Agent, and a
 *          multi-page Streamlit dashboard.
 * Domain: Manufacturing / Supply Chain
 * 
 * This is the standalone NetworkX setup — designed for TRIAL ACCOUNTS and
 * accounts that do not support External Access Integrations or GPU compute
 * pools. All analysis runs on standard warehouse compute using NetworkX
 * graph analytics (no PyTorch Geometric / no GPU required).
 * 
 * What This Script Does:
 * 1.  Creates role, warehouse, database, and schema
 * 2.  Configures Snowflake Intelligence and Cortex
 * 3.  Creates Git integration and repository
 * 4.  Creates stages
 * 5.  Creates tables (input ERP data + NetworkX output tables)
 * 6.  Generates synthetic EV battery supply chain data
 * 7.  Deploys NetworkX notebook (standard warehouse, no EAI)
 * 8.  Uploads semantic model for Cortex Analyst
 * 9.  Deploys NetworkX Streamlit dashboard
 * 10. Creates NetworkX compatibility views
 * 11. Creates Cortex Agent
 * 
 * Prerequisites:
 *   - Snowflake account with ACCOUNTADMIN role
 *   - Cortex AI features enabled
 *   - No GPU or External Access Integration required
 * 
 ******************************************************************************/
...
--------------------------------------------------------------------------------
-- STEP 5: GIT REPOSITORY INTEGRATION
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- STEP 7: CREATE TABLES - ERP DATA (Input)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- STEP 8: CREATE TABLES - NETWORKX MODEL OUTPUTS
--------------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS NX_RISK_SCORES (
...
COMMENT = 'NetworkX-computed risk scores for all supply chain nodes';

CREATE TABLE IF NOT EXISTS NX_PREDICTED_LINKS (
...
COMMENT = 'NetworkX-predicted unknown supplier relationships';

CREATE TABLE IF NOT EXISTS NX_BOTTLENECKS (
    ENTITY_TYPE VARCHAR(20) NOT NULL,
    ENTITY_ID VARCHAR(50) NOT NULL,
    BETWEENNESS_SCORE FLOAT,
    PAGERANK_SCORE FLOAT,
...
COMMENT = 'NetworkX-identified single points of failure in supply chain';

--------------------------------------------------------------------------------
-- STEP 9: GENERATE AND LOAD SYNTHETIC DATA
-- Creates a Snowpark Python stored procedure that generates realistic EV battery
-- supply chain data directly into tables. No CSV files needed.
...
--------------------------------------------------------------------------------
-- STEP 10: CREATE RISK ANALYSIS UDF (NX Edition)
-- Queries NX base tables directly using their native schema (ENTITY_ID).
--------------------------------------------------------------------------------
...
SELECT OBJECT_CONSTRUCT(
    ...
    'analysis', CASE 
        WHEN scenario_type = 'REGIONAL_DISRUPTION' THEN
            ...
            FROM VENDORS v
            JOIN NX_RISK_SCORES rs ON v.VENDOR_ID = rs.ENTITY_ID
        ...
        WHEN scenario_type = 'VENDOR_FAILURE' THEN
            (SELECT OBJECT_CONSTRUCT(
                ...
                'bottleneck_impact', COALESCE(b.RISK_SCORE, 0),
                'recommendation', CASE
            ...
            FROM VENDORS v
            JOIN NX_RISK_SCORES rs ON v.VENDOR_ID = rs.ENTITY_ID
            LEFT JOIN NX_BOTTLENECKS b ON v.VENDOR_ID = b.ENTITY_ID
            WHERE v.VENDOR_ID = target_vendor
            )
        WHEN scenario_type = 'PORTFOLIO_SUMMARY' THEN
            (SELECT OBJECT_CONSTRUCT(
                'total_vendors', COUNT(DISTINCT VENDOR_ID),
                'critical_count', SUM(CASE WHEN rs.RISK_SCORE >= 0.6 THEN 1 ELSE 0 END),
                'high_risk_count', SUM(CASE WHEN rs.RISK_SCORE >= 0.5 THEN 1 ELSE 0 END),
                'avg_portfolio_risk', ROUND(AVG(rs.RISK_SCORE), 3),
                'total_bottlenecks', (SELECT COUNT(DISTINCT ENTITY_ID) FROM NX_BOTTLENECKS),
...
--------------------------------------------------------------------------------
-- STEP 11: FETCH GIT REPOSITORY (for notebooks, streamlit, semantic model)
--------------------------------------------------------------------------------

ALTER GIT REPOSITORY SUPPLY_CHAIN_RISK_REPO FETCH;

--------------------------------------------------------------------------------
-- STEP 11: DEPLOY NETWORKX NOTEBOOK
--------------------------------------------------------------------------------

COPY FILES
    INTO @MODELS_STAGE/networkx/
    FROM @SUPPLY_CHAIN_RISK_REPO/branches/main/notebooks/
    FILES = ('graphsage_supply_chain_risk.ipynb', 'environment.yml');

CREATE OR REPLACE NOTEBOOK SUPPLY_CHAIN_RISK.SUPPLY_CHAIN_RISK.SUPPLY_CHAIN_RISK_NX_NOTEBOOK
    FROM '@MODELS_STAGE/networkx/'
    MAIN_FILE = 'graphsage_supply_chain_risk.ipynb'
    QUERY_WAREHOUSE = 'SUPPLY_CHAIN_RISK_WH'
    IDLE_AUTO_SHUTDOWN_TIME_SECONDS = 1800
    COMMENT = 'Supply Chain Risk Analysis - NetworkX (no external access required)';

ALTER NOTEBOOK SUPPLY_CHAIN_RISK_NX_NOTEBOOK ADD LIVE VERSION FROM LAST;

--------------------------------------------------------------------------------
-- STEP 12: UPLOAD SEMANTIC MODEL
--------------------------------------------------------------------------------

COPY FILES
    INTO @SEMANTIC_MODELS/
    FROM @SUPPLY_CHAIN_RISK_REPO/branches/main/scripts/semantic_models/
    FILES = ('supply_chain_risk_nx.yaml');

--------------------------------------------------------------------------------
-- STEP 13: DEPLOY NETWORKX STREAMLIT APP
--------------------------------------------------------------------------------

CREATE OR REPLACE STREAMLIT SUPPLY_CHAIN_RISK.SUPPLY_CHAIN_RISK.NX_SUPPLY_CHAIN_RISK_APP
    FROM '@SUPPLY_CHAIN_RISK_REPO/branches/main/streamlit_networkx/'
    MAIN_FILE = 'streamlit_app.py'
    QUERY_WAREHOUSE = 'SUPPLY_CHAIN_RISK_WH'
    COMMENT = '{"origin":"sf_sit-is", "name":"supply_chain_risk_intelligence_with_snowflake", "version":{"major":1, "minor":0}}';

ALTER STREAMLIT NX_SUPPLY_CHAIN_RISK_APP ADD LIVE VERSION FROM LAST;

--------------------------------------------------------------------------------
-- STEP 14: CREATE NETWORKX COMPATIBILITY VIEWS
-- These views translate the NetworkX NX_* table schema to the column names
-- expected by the NetworkX Streamlit app. They also enrich bottleneck data by
-- discovering external Tier-2 suppliers from trade data.
-- NX views reference NX_* tables (not base tables), so they always compile.
--------------------------------------------------------------------------------

CREATE OR REPLACE VIEW RISK_SCORES_NX AS
SELECT
    ENTITY_ID AS NODE_ID,
    CASE ENTITY_TYPE
        WHEN 'VENDOR' THEN 'SUPPLIER'
        WHEN 'MATERIAL' THEN 'PART'
        ELSE ENTITY_TYPE
    END AS NODE_TYPE,
    RISK_SCORE,
    CASE
        WHEN RISK_SCORE >= 0.6 THEN 'CRITICAL'
    ...
    MODEL_VERSION,
    COMPUTED_AT AS CALCULATED_AT
FROM NX_RISK_SCORES;

CREATE OR REPLACE VIEW PREDICTED_LINKS_NX AS
SELECT
    pl.SOURCE_ID AS SOURCE_NODE_ID,
    pl.SOURCE_TYPE AS SOURCE_NODE_TYPE,
    pl.TARGET_ID AS TARGET_NODE_ID,
    pl.TARGET_TYPE AS TARGET_NODE_TYPE,
    pl.PREDICTION_METHOD AS LINK_TYPE,
    pl.SIMILARITY_SCORE AS PROBABILITY,
    CASE WHEN pl.SIMILARITY_SCORE >= 0.5 THEN 'STRONG' WHEN pl.SIMILARITY_SCORE >= 0.3 THEN 'MODERATE' ELSE 'WEAK' END AS EVIDENCE_STRENGTH,
    NULL::VARIANT AS SUPPORTING_DATA,
    pl.MODEL_VERSION,
    pl.PREDICTED_AT
FROM NX_PREDICTED_LINKS pl
UNION ALL
SELECT
    pl.TARGET_ID, pl.TARGET_TYPE,
    pl.SOURCE_ID, pl.SOURCE_TYPE,
    pl.PREDICTION_METHOD, pl.SIMILARITY_SCORE,
    CASE WHEN pl.SIMILARITY_SCORE >= 0.5 THEN 'STRONG' WHEN pl.SIMILARITY_SCORE >= 0.3 THEN 'MODERATE' ELSE 'WEAK' END,
    NULL::VARIANT, pl.MODEL_VERSION, pl.PREDICTED_AT
FROM NX_PREDICTED_LINKS pl
UNION ALL
SELECT
    td.SHIPPER_NAME AS SOURCE_NODE_ID,
    'EXTERNAL_SUPPLIER' AS SOURCE_NODE_TYPE,
    v.VENDOR_ID AS TARGET_NODE_ID,
    'VENDOR' AS TARGET_NODE_TYPE,
    'TRADE_EVIDENCE' AS LINK_TYPE,
    LEAST(0.95, 0.6 + COUNT(*) * 0.05) AS PROBABILITY,
    CASE WHEN COUNT(*) >= 3 THEN 'STRONG' WHEN COUNT(*) >= 2 THEN 'MODERATE' ELSE 'WEAK' END AS EVIDENCE_STRENGTH,
    NULL::VARIANT AS SUPPORTING_DATA,
    'v1.0.0-networkx' AS MODEL_VERSION,
    MAX(td.SHIP_DATE)::TIMESTAMP_NTZ AS PREDICTED_AT
FROM TRADE_DATA td
JOIN VENDORS v ON td.CONSIGNEE_NAME = v.NAME
GROUP BY td.SHIPPER_NAME, v.VENDOR_ID;

CREATE OR REPLACE VIEW BOTTLENECKS_NX AS
SELECT
    td.SHIPPER_NAME AS NODE_ID,
    'EXTERNAL_SUPPLIER' AS NODE_TYPE,
    COUNT(DISTINCT v.VENDOR_ID)::NUMBER AS DEPENDENT_COUNT,
    NULL::ARRAY AS DEPENDENT_NODES,
    LEAST(1.0,
        COUNT(DISTINCT v.VENDOR_ID) * 0.04
        + AVG(COALESCE(rs.RISK_SCORE, 0.5)) * 0.2
    ) AS IMPACT_SCORE,
    td.SHIPPER_NAME || ' — external Tier-2 supplier with '
        || COUNT(DISTINCT v.VENDOR_ID)::VARCHAR || ' dependent Tier-1 vendors across '
        || COUNT(DISTINCT v.COUNTRY_CODE)::VARCHAR || ' countries' AS DESCRIPTION,
    'UNMITIGATED' AS MITIGATION_STATUS,
    MAX(td.SHIP_DATE)::TIMESTAMP_NTZ AS IDENTIFIED_AT
FROM TRADE_DATA td
JOIN VENDORS v ON td.CONSIGNEE_NAME = v.NAME
LEFT JOIN NX_RISK_SCORES rs
    ON v.VENDOR_ID = rs.ENTITY_ID AND rs.ENTITY_TYPE = 'VENDOR'
GROUP BY td.SHIPPER_NAME
UNION ALL
SELECT
    b.ENTITY_ID AS NODE_ID,
    'SUPPLIER' AS NODE_TYPE,
    COALESCE(lc.LINK_COUNT, 0)::NUMBER AS DEPENDENT_COUNT,
    NULL::ARRAY AS DEPENDENT_NODES,
    b.RISK_SCORE AS IMPACT_SCORE,
    COALESCE(v.NAME, b.ENTITY_ID) || ' — high betweenness centrality, critical supply chain path (Rank #' || b.BOTTLENECK_RANK || ')' AS DESCRIPTION,
    'UNMITIGATED' AS MITIGATION_STATUS,
    b.IDENTIFIED_AT
FROM NX_BOTTLENECKS b
LEFT JOIN VENDORS v ON b.ENTITY_ID = v.VENDOR_ID
LEFT JOIN (
    SELECT NODE_ID, COUNT(DISTINCT PEER_ID) AS LINK_COUNT FROM (
        SELECT SOURCE_ID AS NODE_ID, TARGET_ID AS PEER_ID FROM NX_PREDICTED_LINKS
        UNION ALL
        SELECT TARGET_ID AS NODE_ID, SOURCE_ID AS PEER_ID FROM NX_PREDICTED_LINKS
    ) sub
    GROUP BY NODE_ID
) lc ON b.ENTITY_ID = lc.NODE_ID
WHERE b.ENTITY_TYPE = 'VENDOR';

--------------------------------------------------------------------------------
-- STEP 15: CREATE CORTEX AGENT (LAST - after all dependencies)
--------------------------------------------------------------------------------

CREATE OR REPLACE AGENT SUPPLY_CHAIN_NX_AGENT
  COMMENT = 'Supply Chain Risk Copilot (NetworkX) - answers questions using NetworkX semantic model and scenario analysis'
  FROM SPECIFICATION
  $$
  models:
  ...
  instructions:
    system: |
      You are a Supply Chain Risk Analyst powered by NetworkX graph analytics.
  ...
  tool_resources:
    supply_chain_analytics:
      semantic_model_file: "@SUPPLY_CHAIN_RISK.SUPPLY_CHAIN_RISK.SEMANTIC_MODELS/supply_chain_risk_nx.yaml"
...
ALTER SNOWFLAKE INTELLIGENCE SNOWFLAKE_INTELLIGENCE_OBJECT_DEFAULT
  ADD AGENT SUPPLY_CHAIN_NX_AGENT;

--------------------------------------------------------------------------------
-- SETUP COMPLETE
--------------------------------------------------------------------------------

SELECT 'NetworkX Supply Chain Risk Analysis - Setup Complete!' as STATUS;

/******************************************************************************
 * SETUP COMPLETE! (NetworkX Edition - Trial Account Compatible)
 * 
 * Your NetworkX Supply Chain Risk Analysis platform is ready with:
 * - Database: SUPPLY_CHAIN_RISK
 * - Warehouse: SUPPLY_CHAIN_RISK_WH
 * - Tables: 6 input + 3 NX output = 9 tables
 * - Views: 3 NX views (RISK_SCORES_NX, PREDICTED_LINKS_NX, BOTTLENECKS_NX)
 * - NetworkX Notebook: SUPPLY_CHAIN_RISK_NX_NOTEBOOK (standard warehouse)
 * - NetworkX Streamlit: NX_SUPPLY_CHAIN_RISK_APP
 * - NX Agent: SUPPLY_CHAIN_NX_AGENT (semantic model on _NX views)
 * 
 * No GPU compute pool or External Access Integration required.
 * 
 * NEXT STEPS:
 * 1. Open the NetworkX notebook in Snowsight and run all cells to compute
 *    risk scores, predicted links, and bottleneck analysis.
 *    The notebook automatically switches to SUPPLY_CHAIN_RISK_ROLE so all
 *    output tables are owned by the demo role (no ACCOUNTADMIN dependency).
 * 2. Open the NetworkX Streamlit dashboard to explore results
