...
def load_key_metrics(_session):
    ...
    queries = {
        'total_nodes': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.RISK_SCORES_GNN",
        'critical_count': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.RISK_SCORES_GNN WHERE RISK_CATEGORY = 'CRITICAL'",
        'bottleneck_count': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.BOTTLENECKS_GNN",
        'predicted_links': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.PREDICTED_LINKS_GNN"
    }
...
def load_illusion_data(_session):
    ...
    try:
        bottleneck = _session.sql(f"""
            ...
            FROM {DB_SCHEMA}.BOTTLENECKS_GNN
        ...
        dependent_vendors = _session.sql(f"""
            SELECT 
                ...
                pl.PROBABILITY,
                rs.RISK_SCORE
            FROM {DB_SCHEMA}.PREDICTED_LINKS_GNN pl
            LEFT JOIN {DB_SCHEMA}.VENDORS v ON pl.TARGET_NODE_ID = v.VENDOR_ID
            LEFT JOIN {DB_SCHEMA}.RISK_SCORES_GNN rs ON v.VENDOR_ID = rs.NODE_ID
...
def main():
    ...
    if metrics['total_nodes'] == 0:
        ...
        <div style="background: linear-gradient(135deg, #1e3a5f 0%, #0f172a 100%); 
            ...
            <a href="../notebooks/SUPPLY_CHAIN_RISK.SUPPLY_CHAIN_RISK.SUPPLY_CHAIN_RISK_NOTEBOOK" 
               target="_blank"
    ...
    <div style="background: linear-gradient(135deg, rgba(59, 130, 246, 0.1) 0%, rgba(139, 92, 246, 0.1) 100%);
        ...
        <p style="color: #94a3b8; margin-top: 0.5rem; font-size: 0.9rem;">
            The GNN analysis below reveals these unknown dependencies.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if bottleneck and len(nodes) > 1:
        ...
        <p style="text-align: center; color: #64748b; font-size: 0.9rem; margin-top: 1rem;">
            ...
            <strong>Red node:</strong> common Tier-2 supplier (discovered by GNN) &nbsp;|&nbsp;
            <strong>Dashed lines:</strong> Predicted dependencies
        </p>
        """, unsafe_allow_html=True)
    else:
        st.info("Run the GNN notebook to discover unknown dependencies in your supply chain data.")
