...
def load_discovery_summary(_session):
    ...
    queries = {
        'total_bottlenecks': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.BOTTLENECKS_GNN",
        'critical_bottlenecks': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.BOTTLENECKS_GNN WHERE IMPACT_SCORE >= 0.7",
        'predicted_links': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.PREDICTED_LINKS_GNN",
        'high_confidence_links': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.PREDICTED_LINKS_GNN WHERE PROBABILITY >= 0.7",
        'total_dependencies': f"SELECT COALESCE(SUM(DEPENDENT_COUNT), 0) as CNT FROM {DB_SCHEMA}.BOTTLENECKS_GNN"
    }
    ...
    try:
        result = _session.sql(f"""
            SELECT 
                ...
                EVIDENCE_STRENGTH,
                PREDICTED_AT
            FROM {DB_SCHEMA}.PREDICTED_LINKS_GNN
        ...
        for idx, row in bottlenecks.iterrows():
            ...
            <div class="bottleneck-card {impact_class}">
                ...
                <div class="bottleneck-description">
                    {row.get('DESCRIPTION', 'Unknown supply chain convergence point requiring attention.')}
                </div>
                <div class="evidence-section">
