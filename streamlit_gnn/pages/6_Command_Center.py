    ...
    queries = {
        'critical_suppliers': f"""
            SELECT 
                ...
                rs.RISK_CATEGORY,
                rs.UPDATED_AT
            FROM {DB_SCHEMA}.RISK_SCORES_GNN rs
...
def load_alert_summary(_session):
    ...
    queries = {
        'critical_count': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.RISK_SCORES_GNN WHERE RISK_CATEGORY = 'CRITICAL' AND NODE_TYPE = 'SUPPLIER'",
        'high_count': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.RISK_SCORES_GNN WHERE RISK_CATEGORY = 'HIGH' AND NODE_TYPE = 'SUPPLIER'",
        'medium_count': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.RISK_SCORES_GNN WHERE RISK_CATEGORY = 'MEDIUM' AND NODE_TYPE = 'SUPPLIER'",
        'bottleneck_count': f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.BOTTLENECKS_GNN WHERE IMPACT_SCORE >= 0.5"
    }
